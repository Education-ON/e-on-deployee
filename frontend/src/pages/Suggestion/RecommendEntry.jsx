    ...
    (async () => {
      try {
        ...
        try {
          ...
          const { data } = await axios.get("/api/interests/myInterests");
          // 예: { my: [...] } 또는 [...]
          const arr =
            Array.isArray(data?.my) ? data.my :
        ...
        if (complete) {
          ...
          navigate(`/onboarding/preferences?redirect=${encodeURIComponent(next)}`, { replace: true });
        }
      } catch {
        // 미로그인 → 로그인
