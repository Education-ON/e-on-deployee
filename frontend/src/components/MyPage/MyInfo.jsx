...
const MyInfo = () => {
  ...
  const isSocial = ["kakao", "naver", "google"].includes(user?.provider || "");
  ...
  useEffect(() => {
    if (user) {
      ...
      const notif = user.email_notification ?? user.emailNotification ?? false;
      setEmailNotification(!!notif);

      user.type === "student" ? setUserType("학생") : setUserType("학부모");
  ...
  const handleVerifyPassword = async (e) => {
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: err.response?.data?.message || "비밀번호가 일치하지 않습니다.",
      });
    }
  };
  ...
  const handleRequestCode = async () => {
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: err.response?.data?.message || "코드 요청에 실패했습니다.",
      });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isSocial) {
        // 소셜은 code 필수
        if (!code) {
          setMessage({ type: "error", text: "이메일 인증 코드를 입력하세요." });
          return;
        }
        payload.code = code;
      } else {
        ...
        if (!currentPassword) {
          setMessage({ type: "error", text: "현재 비밀번호를 입력하세요." });
          return;
        }
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <div className={styles.formGroup}>
            ...
            <div className={styles.ageInputWrapper}>
              <button
                type="button"
                className={`${styles.ageButton} ${age <= 8 ? styles.disabled : ""}`}
                onClick={() => setAge((prev) => Math.max(8, prev - 1))}
                disabled={age <= 8}
              >
                -
              </button>
              <input
                ...
                onChange={(e) => {
                  ...
                  if (!isNaN(value)) {
                    setAge(Math.max(8, Math.min(16, value)));
                  }
                }}
              />
              <button
                type="button"
                className={`${styles.ageButton} ${age >= 16 ? styles.disabled : ""}`}
                onClick={() => setAge((prev) => Math.min(16, prev + 1))}
                disabled={age >= 16}
              >
                +
              </button>
          ...
          <div className={styles.formGroup}>
            <label>회원 종류</label>
            <input className={styles.input} type="text" value={userType} disabled />
          </div>
